...
file_backed_swap_in (struct page *page, void *kva) {
	...
	if (file_read_at(file_page->file, kva, file_page->length, file_page->offset) != (int)file_page->length) return false;
...
file_backed_swap_out (struct page *page) {
	...
	if (pml4_is_dirty(curr->pml4, page->va))
	{
		...
		if (check != file_page->length) return false;
...
do_mmap(void *addr, size_t length, int writable, struct file *file, off_t offset)
{
	...
	if (mmap_file->file == NULL) return NULL;
	...
	while (read_bytes > 0 || zero_bytes > 0)
	{
		...
		if (!vm_alloc_page_with_initializer(VM_FILE, upage, writable, lazy_load_file, file_info)) return false;

		struct page *page = NULL;
		if ((page = spt_find_page(&curr->spt, upage)) == NULL) return false;
...
void do_munmap(void *addr)
{
	...
	while (temp_elem != list_tail(&mmap_file->page_list))
	{
